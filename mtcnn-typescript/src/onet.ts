import { Mat } from '@techstark/opencv-js'
import { Box } from './shared/Box'
import { extractImagePatches } from './mtcnn/extractImagePatches'
import { ONET_MODEL_PATH, ONET_INPUT_SIZE } from './shared/globals'
import { Tensor } from 'onnxruntime-web'
import { loadMtcnnModel } from './utils/loadMtcnnModel'
import { MtcnnBox } from './mtcnn/MtcnnBox'
import { Point } from './shared/Point'
import { nonMaxSuppression } from './utils/nonMaxSuppression'

export type OnetOutput = {
  dense_5: Tensor,
  softmax_2: Tensor
  dense_6: Tensor
}

export async function run(
  image: Mat,
  inputBoxes: Box[],
  scoreThreshold: number,
) {
  const rnetInputs = await extractImagePatches(image, inputBoxes, { width: ONET_INPUT_SIZE, height: ONET_INPUT_SIZE })
  console.log("rnetInputs", rnetInputs, inputBoxes)
  const batchSize = rnetInputs.length;
  const height = ONET_INPUT_SIZE;  // Height for RNet
  const width = ONET_INPUT_SIZE;   // Width for RNet
  const channels = 3; // Number of channels (RGB)

  // Create a single array to hold all batched data
  const batchedData = new Float32Array(batchSize * height * width * channels);
  rnetInputs.forEach((imageData, index) => {
    batchedData.set(imageData, index * height * width * channels);
  });

  const inputTensor = new Tensor('float32', batchedData, [batchSize, height, width, channels]);
  const feeds = { 'input_3': inputTensor }; // Use the correct input name for RNet
  const session = await loadMtcnnModel(ONET_MODEL_PATH);
  const onetOutput = await session.run(feeds) as OnetOutput;

  const landmarksData = onetOutput.dense_6.data;
  const scoreData = onetOutput.softmax_2.data
  const regionsData = onetOutput.dense_5.data

  const regions: { box: Box, score: number, points: Point[] }[] = [];
  for (let i = 0; i < regionsData.length / 4; i++) {
    const landmarkPoints = [];
    let score = scoreData[i * 2 + 1] as number; // Assuming the second score is the face confidence
    if (score > scoreThreshold) {
      const refinementSlice = regionsData.slice(i * 4, i * 4 + 4) as Float32Array;
      const refinement = new MtcnnBox(
        refinementSlice[0],
        refinementSlice[1],
        refinementSlice[2],
        refinementSlice[3]
      )
      const box = inputBoxes[i].calibrate(refinement);
      for (let j = 0; j < 5; j++) {
        const lx = landmarksData[i * 10 + j] as number * (box.width + 1) + box.x;
        const ly = landmarksData[i * 10 + j + 5] as number * (box.height + 1) + box.y;

        landmarkPoints.push(new Point(lx, ly));
      }
      regions.push({ box, score, points: landmarkPoints });
    }
  }
  const filteredBoxes = regions.map(region => region.box)
  const filteredScores = regions.map(region => region.score)
  const filteredPoints = regions.map(region => region.points)
  const nmsIndices = nonMaxSuppression(
    filteredBoxes,
    filteredScores,
    0.7,
    false
  )
  const points = nmsIndices.map(idx => filteredPoints[idx])
  const finalBoxes = nmsIndices.map(idx => filteredBoxes[idx])
  const finalScores = nmsIndices.map(idx => filteredScores[idx])

  return {
    boxes: finalBoxes,
    scores: finalScores,
    points
  };
}